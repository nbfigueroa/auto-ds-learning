...
mat_data_dir = '../../mat/franka-cooking-dec21/';
matfile = strcat(mat_data_dir,'demos_dec2021_raw_data.mat');
load(matfile);

%%%%%% Plot Franka Cooking Workspace (need rosbag_to_mat repo)
figure('Color',[1 1 1])
...
for i=1:length(data)
    ...
    fid = fopen(save_dir + "franka_cooking_traj"+num2str(nfiles+i,'%02d')+".json",'w');
...
% delete(save_dir + "*");
save(save_dir + "franka_cooking_traj.mat", 'data')
...
for i=1:length(segs)
    save_file = fileparts(matlab.desktop.editor.getActiveFilename) + "/TrajData/franka_cooking_traj" + ...
        num2str(i,'%02d') + ".mat";
    seg = segs{i};
    save(save_file, 'seg')
end
...
figure('Color',[1 1 1])
% Plot Franka Cooking Workspace
plotFrankaCookingWorkspace_extended(H_left_bowl, H_center_bowl, H_right_bowl);
...
save_file = fileparts(matlab.desktop.editor.getActiveFilename) + "/SegData-DS/franka_cooking_traj.mat";
