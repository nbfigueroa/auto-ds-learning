...
data_file = pkg_dir + "/SegData-DS/mitsubishi_traj.mat";
load(data_file);
...
%     DS_name = strcat('mitsubishi_ds_', num2str(s));
    DS_name = strcat('mitsubishi_ds_A_', num2str(s));
...
% save_file = pkg_dir + "/models/mitsubishi_sequenceDS_nov2021update.mat";
save_file = pkg_dir + "/models/mitsubishi_sequenceDS_nov2021update_A.mat";
...
% Change workspace object locations
change_object_locs = 0;

if change_object_locs 
    object_config_idx = randi([1 4]);
else
    object_config_idx = 1;
end

offsets_scanner   = [0  0    0    0.10; ... 
                     0  0    0.05 0.10; ...
                     0  0    0    0];
offsets_proc      = [0  0.0 -0.1  0.13; ...
                     0  0.25  0.03 0.13; ...
                     0   0    0    0];
offsets_check     = [0 -0.10 0  -0.25;
                     0 -0.30    0  -0.15;...
                     0  0    0   0];
workspace_objects{1}.H(1:3,4,2) = workspace_objects{1}.H(1:3,4,2) + offsets_scanner(:,object_config_idx); 
workspace_objects{1}.H(1:3,4,3) = workspace_objects{1}.H(1:3,4,3) + offsets_proc(:,object_config_idx);
workspace_objects{1}.H(1:3,4,4) = workspace_objects{1}.H(1:3,4,4) + offsets_check(:,object_config_idx);
...
make_vid = 1;
if make_vid
    vid_name = pkg_dir + "/figs/mitsubishi_sequenceDS_simulation";
...
for s=1:N_ds-1
    ...
    % Randomly define which final DS to use
    if s > (N_ds-2);s_seq = s + randi([0 1]); else; s_seq = s;end
    
    % Construct DS with shifted attractor if needed
    ds_att_s      = sequence_ds{s_seq}.att; 
    ds_att_s_hat  = ds_att_s;
    if s_seq == 1 % Shift attractors in picking tray
         ds_att_s_hat  = sequence_ds{s_seq}.att; 
%          + [0.001*randi([0 5]); 0.001*randi([0 5]); 0];
    elseif s_seq ==  2
        ds_att_s_hat  = ds_att_s + offsets_scanner(:,object_config_idx);        
    elseif s_seq ==  3
        ds_att_s_hat  = ds_att_s + offsets_proc(:,object_config_idx);    
    elseif s_seq ==  4
        ds_att_s_hat  = ds_att_s + offsets_check(:,object_config_idx);
    end
    
    % Store latest attractor used
    sequence_ds{s_seq}.att_new =  ds_att_s_hat;
 
    Rot_att = eye(3);
    
    if object_config_idx == 2
        if s_seq == 3
            Rot_att = rotationMatrx('z', deg2rad(-45));
        end
        if s_seq == 4
            Rot_att = rotationMatrx('z', deg2rad(180));
        end
%         if s_seq == 5
%             Rot_att = rotationMatrx('z', deg2rad(30));
%         end  
    elseif object_config_idx == 4
        if s_seq == 4
            Rot_att = rotationMatrx('z', deg2rad(95));
        end
        if s_seq == 5
            Rot_att = rotationMatrx('z', deg2rad(-45));
        end    
    ...
    if s_seq > (N_ds-2) 
       x0 = sequence_ds{N_ds-2}.att_new;
        ...
        if s_seq> 1
            ...
            x0_id = randi([1 3]);
            x0_all = [0.40 0.40 0.40;-0.20 -0.10 0; 0.5 0.5 0.5];
            x0 = x0_all(:,x0_id);
        end
    end
